...
class hashed():
	def __init__(self, mc, gpu_id=0, hashed=True, retraining=False):
		...
		with tf.device('/gpu:{}'.format(gpu_id)):
			...
			#make Tensor
			self._add_forward_graph(retraining)
	...
	#---------------------------------------------------------------------------------
	def _add_forward_graph(self, retraining=False):
		"""NN architecture."""
		...
		if mc.LOAD_PRETRAINED_MODEL:
			...
			assert tf.gfile.Exists(mc.PRETRAINED_INFO_PATH), 'Cannot find pretrained model at the given path:' '  {}'.format(mc.PRETRAINED_INFO_PATH)
		if retraining :
			print("OK to save", mc.RETRAINED_INFO_PATH)
			self.caffemodel_weight = joblib.load(mc.RETRAINED_MODEL_PATH)
			self.caffemodel_info = joblib.load(mc.RETRAINED_INFO_PATH)
			
		else :
			self.caffemodel_weight = joblib.load(mc.PRETRAINED_MODEL_PATH)
			self.caffemodel_info = joblib.load(mc.PRETRAINED_INFO_PATH)

		dense1 = self._hashed_fc_layer('dense1', self.image_input, hiddens=1000, flatten=True, sparse_ratio=mc.SPARSE_RATIO)

		self.preds = self._hashed_fc_layer('dense2', dense1, hiddens=10, flatten=False, relu=False, sparse_ratio=mc.SPARSE_RATIO)
	...
	def _hashed_fc_layer(
      ...
      xavier=False, stddev=0.001, sparse_ratio=0.5):
		mc = self.mc
		...
		with tf.variable_scope(layer_name) as scope:
			...
			kmeans = Pruned(cWeights=kernel_val, XXarray=index_array,  XXnCluster=before_nCentroid,blocked=block, blocked_param=num_block, sparse_ratio=sparse_ratio);

			self.hash_index[layer_name] = kmeans.label();
			self.hash_num[layer_name] = kmeans.num_centro();
			self.mask[layer_name] = kmeans.mask();
			self.blocked[layer_name] = block;
			self.num_block[layer_name] = num_block;
