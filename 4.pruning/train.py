...
tf.app.flags.DEFINE_string('pretrained_model_path', '../3.posttrain/MNIST/train/MNIST_POSTTRAIN.pkl',
                           """Path to the pretrained model.""")
tf.app.flags.DEFINE_string('pretrained_info_path', '../3.posttrain/MNIST/train/MNIST_POSTTRAIN_INFO.pkl',
                           """Path to the pretrained model.""")
tf.app.flags.DEFINE_string('retrained_model_path', './MNIST/train/MNIST_PRUNING.pkl',
                           """Path to the pretrained model.""")
tf.app.flags.DEFINE_string('retrained_info_path', './MNIST/train/MNIST_PRUNING_INFO.pkl',
                           """Path to the pretrained model.""")
...
#---------------------------------------------------------------------------------
def train(sparse_ratio = 0.9, max_step=1000, retraining=False):
	...
	with tf.Graph().as_default():
		...
		if FLAGS.net == 'hashed':
			...
			mc.SPARSE_RATIO=sparse_ratio;
			mc.MAX_STEP=max_step;
			if retraining :
				mc.RETRAINED_MODEL_PATH = FLAGS.retrained_model_path
				mc.RETRAINED_INFO_PATH = FLAGS.retrained_info_path
			model = hashed(mc, gpu_id=FLAGS.gpu, hashed=FLAGS.hashed, retraining=retraining) # 2.hashed.py
		...
		def TK_TRANSFORM(grad, hash_num, hash_index, mask):
			...
			for i in xrange(grad.shape[0]):
				for i2 in xrange(grad.shape[1]):
					temp_grad[i][i2]=temp_centroid[hash_index[i][i2]] * mask[hash_index[i][i2]]
			return temp_grad;
		#-----------------------------------------------------------------
				...
				if FLAGS.hashed : 
					...
					for i in xrange(len(model.grads_placeholder)):
						...
						if str(model.grads_placeholder[i][1].name.split('/')[0]) in model.hash_num.keys():
							...
							mask = model.mask[model.grads_placeholder[i][1].name.split('/')[0]];
							...
							feed_dict[model.grads_placeholder[i][0]] = TK_TRANSFORM(grads[i], hash_num, hash_index, mask);
		...
		for tuple_tem in save_tuple.values():
			...
			for tuple_temp2 in tuple_tem:
				print('tuple_tem value is', tuple_temp2.shape)
		...
		print("tk2 is ", save_tuple2['dense2'][2])		
		print("tk2 is ", save_tuple2['dense2'][3])		
		...
		import pickle
		#os.getcwd()
		if tf.gfile.Exists(os.path.join(FLAGS.train_dir, 'MNIST_PRUNING.pkl')):
			os.remove(os.path.join(FLAGS.train_dir, 'MNIST_PRUNING.pkl'));

		with open(os.path.join(FLAGS.train_dir, 'MNIST_PRUNING.pkl'), 'wb') as f:
			pickle.dump(save_tuple, f);

		if tf.gfile.Exists(os.path.join(FLAGS.train_dir, 'MNIST_PRUNING_INFO.pkl')):
			os.remove(os.path.join(FLAGS.train_dir, 'MNIST_PRUNING_INFO.pkl'));

		with open(os.path.join(FLAGS.train_dir, 'MNIST_PRUNING_INFO.pkl'), 'wb') as f:
...
def main(argv=None):  # pylint: disable=unused-argument
	...
	'''
	# caffe : 
	# layer [0.33, 0.8, 0.9, 0.8]
	# iter [500, 1000, 10500, 11000, 500]
	
	#max_step= [500, 1000, 1000, 1000, 1000]
	max_step= [1000, 1000, 1000, 1000, 1000]
	#sparse_ratio = [0.8, 0.7, 0.6, 0.5, 0.4]
	sparse_ratio = [0.75, 0.75, 0.75, 0.75, 0.75]
	if tf.gfile.Exists(FLAGS.train_dir):
		train(0.9, 1000)
		for i in range(len(sparse_ratio)):
			print("============================================={}===========================================".format(sparse_ratio[i]))
			train(sparse_ratio[i], max_step[i], retraining=True);
	else:
		tf.gfile.MakeDirs(FLAGS.train_dir)
		train(0.9)
		for i in range(5):
