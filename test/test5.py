...
# pre-define model
assert tf.gfile.Exists("../posttrain/MNIST/train/MNIST_POSTTRAIN.pkl"), 'Cannot find pretrained model at the given path:' '  {}'.format("../posttrain/MNIST/train/MNIST_POSTTRAIN.pkl")
caffemodel_weight = joblib.load("../posttrain/MNIST/train/MNIST_POSTTRAIN.pkl")

assert tf.gfile.Exists("../posttrain/MNIST/train/MNIST_POSTTRAIN_INFO.pkl"), 'Cannot find pretrained model at the given path:' '  {}'.format("../posttrain/MNIST/train/MNIST_POSTTRAIN_INFO.pkl")
caffemodel_weight_info = joblib.load("../posttrain/MNIST/train/MNIST_POSTTRAIN_INFO.pkl")
...
with tf.variable_scope(layer_name) as scope:
	...
	if use_pretrained_param:
		...
		block=caffemodel_weight_info[layer_name][2]
		num_block=caffemodel_weight_info[layer_name][3]

		
		kmeans = Pruned(cWeights=kernel_val, XXarray=index_array,  XXnCluster=before_nCentroid,blocked=block, blocked_param=num_block, sparse_ratio=0.75);

		np.set_printoptions(threshold=10)
		#kmeans = XXhash(cWeights=kernel_val, nCluster=9800, blocked=True, blocked_param=64)
		print("Kmeans label is", kmeans.label())
		print("Kmeans label is", kmeans.mask())
		#print("Kmeans cluster_centers is", kmeans.centro())
		print("before weight is", kernel_val);
		print("Kmeans label is", kmeans.label())
