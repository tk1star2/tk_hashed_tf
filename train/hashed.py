...
#-----------------------------------------tk---------------------
    ...
    dense1 = self._hashed_layer(
        'dense1', self.image_input, hiddens=1000, flatten=True)
    dense2 = self._hashed_layer(
        'dense2', dense1, hiddens=10, flatten=False)
...
#----------------------------------------------------------------
  ...
  #---------------------------------------------------------------------------
  def XXhash(self, input_key)
	if type(input_key) is int:
		output_value = input_key;
	else:
		output_value = int(input_key);
	return output_value;
