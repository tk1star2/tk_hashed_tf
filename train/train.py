...
def train():
	...
	with tf.Graph().as_default():
		...
		def _load_data(load_to_placeholder=True):
			...
			if load_to_placeholder:
				...
				feed_dict = {image_input: image_per_batch, input_mask: np.reshape(
					sparse_to_dense(mask_indices, [mc.BATCH_SIZE, mc.ANCHORS],[1.0]*len(mask_indices)), [mc.BATCH_SIZE, mc.ANCHORS, 1]),
	...
	for step in xrange(FLAGS.max_steps):
		...
		if step % FLAGS.summary_step == 0:
			...
			op_list = [model.train_op, model.loss, summary_op, model.det_boxes,
		...
		# sess.run
			...
			if mc.NUM_THREAD > 0:
				# sess.run
				_, loss_value, conf_loss, bbox_loss, class_loss = sess.run([model.train_op, model.loss, model.conf_loss, model.bbox_loss, model.class_loss], options=run_options)
			else:
				# sess.run
				feed_dict, _, _, _ = _load_data(load_to_placeholder=False)
				_, loss_value, conf_loss, bbox_loss, class_loss = sess.run([model.train_op, model.loss, model.conf_loss, model.bbox_loss,model.class_loss], feed_dict=feed_dict)
	  #*********************************************************

		duration = time.time() - start_time

		assert not np.isnan(loss_value), 'Model diverged. Total loss: {}, conf_loss: {}, bbox_loss: {}, ' 'class_loss: {}'.format(loss_value, conf_loss, bbox_loss, class_loss)
